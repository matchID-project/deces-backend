...
describe('index.controller.ts - GET request', () => {
  ...
  it('Query by lastSeenAliveDate', async () => {
    ...
    expect(result.response.persons.every(x => parseInt(x.death.date, 10) > 20200120)).toBe(true);
    expect(result.response.persons.length).toBeGreaterThan(0);
  });
...
describe('index.controller.ts - POST request', () => {
  ...
  it('Sort and order by score asc', async () => {
    ...
    expect(result.response.persons.every((x, i) => i === 0 || x.score >= result.response.persons[i - 1].score)).toBe(true)
    expect(result.response.persons.length).toBeGreaterThan(0);
  });
